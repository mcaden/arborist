...
| `branches_list`                 | `repoRoot: string`              | `BranchInfo[]`            | List local and remote-tracking branches for the "From Branch" worktree flow. Discovery failures return an empty list.                                                                                                                                                                                   |
...
| `worktree_pr_info`              | `WorktreePrInfoArgs`            | `WorktreePrInfo`          | Look up the pull/merge request for a worktree's current branch. Provider (GitHub/GitLab/Azure DevOps) is detected from the `origin` remote; data comes from the matching CLI (`gh`/`glab`/`az`). Always resolves: unknown-host / missing-CLI / no-PR are conveyed via `provider`/`cliAvailable`/`note`. |
| `open_external_url`             | `OpenExternalUrlArgs`           | `void`                    | Open a validated `http`/`https` URL in the OS default browser. Non-http(s) schemes are rejected, so it cannot act as a generic file/shell opener.                                                                                                                                                       |
...
| `worktree_create_from_branch`   | `WorktreeCreateFromBranchArgs`  | `WorktreeCreateResult`    | Create `<workspace>/.arborist/.worktrees/<branch>` checking out an existing branch (local, or a new tracking branch from `<remote>/<branch>`) and maybe start prep.                                                                                                                                     |
...
| `worktree_tab_close`            | `WorktreeTabCloseArgs`          | `WorktreeTabCloseResult`  | Cascade-close child sessions/sub-sessions; delete the worktree only if teardown is clean. `subOutcomes` carries per-sub-session `SubSessionCloseResult` so the UI can warn about polite-close prompts, force-kill unconfirmed, or refused shared-editor terminations.                                   |
...
| `subsession_close`              | `SubSessionCloseArgs`           | `SubSessionCloseResult`   | Close/detach/terminate a sub-session according to kind and intent. Returns `{outcome, status, pid?, message?}` so the UI can show outcome-aware copy (e.g. "force-kill unconfirmed", "refused to terminate a shared editor").                                                                           |
