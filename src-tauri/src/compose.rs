//! Pure command composition, label dedup, worktree validation, and shell-quoting helpers (Phase 5 of the implementation plan).
//!
//! This module is **pure**: it performs no filesystem or process I/O. The caller (Phase 7's `session_create`) is responsible for:
//!
//! 1. Calling [`validate_worktree`] to canonicalize the user-supplied path.
//! 2. Calling [`compose_command`] with the canonical worktree path.
//! 3. Materialising the returned legacy [`ComposedInvocation::temp_files`] to disk before spawning the PTY.
//!
//! Keeping composition pure makes it trivially unit-testable and lets `respawn_existing` (Phase 7) re-materialise temp files without re-running
//! composition.
//!
...
use crate::types::{Error, SessionId, TempFileSpec, Tool};

// --------------------------------------------------------------------------- Public API
// ---------------------------------------------------------------------------

/// The product of [`compose_command`]: the verbatim shell string that will be passed to the platform shell as its `-c` / `/c` argument, plus any temp
...
pub struct ComposedInvocation {
    ...
    /// Legacy files the backend must write to disk before spawning the PTY. Current session creation leaves this empty; persisted older sessions may
    /// still carry temp files for restore.
    pub temp_files: Vec<TempFileSpec>,
}

/// Inputs to [`compose_command`]. Borrowed to keep the call site allocation- free; the function itself returns owned data.
pub struct ComposeInputs<'a> {
    pub session_id: SessionId,
    pub tool: Tool,
    /// **Already canonicalized** by [`validate_worktree`]. Composition does not re-canonicalize and never touches disk.
...
/// suffixes it always picks the *lowest* free integer, so e.g. given `[foo, foo 3]` and base `foo` it returns `foo 2`.
#[must_use]
...
///
/// Returns `Ok(name.to_owned())` when valid, or `Err(message)` describing the first rule violated. The rules deliberately mirror git's branch naming
/// rules subset that makes sense for the in-app create-worktree flow:
///
/// * No spaces.
/// * No `..`, `~`, `^`, `:`, `?`, `*`, `[`, `\`.
...
/// * 1–255 characters.
///
/// Pure: no IO. Used both by the Tauri command (server-side check before shelling out to `git worktree add`) and re-implemented identically on the TS
/// side as `validateWorktreeName` for inline picker feedback.
pub fn validate_worktree_name(name: &str) -> Result<String, String> {
    if name.is_empty() {
        return Err("name cannot be empty".to_owned());
    }
    // Use char count (Unicode scalars) on both sides of the boundary to keep Rust and TS rejecting / accepting the same inputs.
    ...
    // Per-component checks: every '/'-separated segment must independently satisfy git's refs(7) rules.
...
///
/// On Windows we go through `dunce::canonicalize` to avoid `\\?\` UNC prefixes that confuse downstream tooling and string comparisons.
...
/// Single source of truth for the path to a Copilot session's OTel JSONL file. Used by [`env_for_tool`] (to point Copilot's exporter at it), by
/// `pty_pool` spawn prep (to wipe a stale copy from a prior run), and by `session_metrics::MetricsRegistry::start` (to tail it). Anywhere else that
/// needs this path **must** call this helper — never reconstruct the filename inline.
#[must_use]
pub fn copilot_otel_path(session_id: &SessionId) -> PathBuf {
    session_temp_dir(session_id).join("otel.jsonl")
}

/// Per-session **environment variables** to inject into the spawned CLI process, separately from the shell command. These are *additions* on top of
/// the parent process's inherited env (the spawner does not call `env_clear`). They are recomputed on every spawn from the session id and **never**
/// stored on `Session` — only the path to the OTel JSONL would be derivable anyway, and persisting it would let stale paths from a previous install
/// leak across upgrades.
///
/// Values are `OsString` so paths with non-UTF-8 segments (rare on Windows but possible) round-trip without lossy `�` substitution.
///
/// Per-tool behaviour:
///
...
///
/// The set is intentionally conservative — other characters (`@ + : = ,` etc.) are also unquoted-safe on both shells, but the conservative floor
/// covers every realistic AI-session id today (UUIDs from both Claude and Copilot) and any value outside it falls through to the host quoter, which
/// is correct, just verbose. Do not expand this set without auditing every shell context the resulting command flows through (`cmd.exe /c`, `.cmd`
/// shim → CRT re-parse, `sh -c`).
...
/// POSIX single-quoted form. Round-trip rule: `sh -c "echo <output>"` prints the original byte sequence unchanged.
///
/// Algorithm (well-known): wrap the value in single quotes, and replace any internal `'` with the four-character sequence `'\''`. This works for
/// every byte except NUL (which no shell can carry as an argument anyway). Empty strings become `''` so they remain a single argument.
#[must_use]
...
/// `cmd.exe` quoting.
///
/// The `cmd.exe` parser splits arguments on whitespace and then invokes the program loader, which re-parses the command tail according to the C
...
///
/// The implementation here errs on the side of over-quoting: every reserved character gets a `^` prefix and the whole value is wrapped in `"…"`. This
/// is verbose but safe, and it's only used for argv values we ourselves supply.
#[must_use]
pub fn shell_quote_cmd(value: &str) -> String {
    ...
    // Now caret-escape cmd.exe metacharacters. Since the whole value is wrapped in `"…"` below, most metacharacters are inert — but `%` and `!` are
    // still expanded inside double quotes, and `^` itself is the escape character so it must be doubled. We caret-escape these three *inside* the
    // quoted region; everything else is safe.
...
pub struct PlatformShell {
    ...
    pub flag: &'static str,
    /// Quoter to use when interpolating dynamic values into the composed command we hand to `program flag`.
    pub quoter: Quoter,
}
...
mod tests {
    ...
    fn inputs<'a>(tool: Tool, worktree: &'a Path, label: &'a str) -> ComposeInputs<'a> {
        ComposeInputs {
            session_id: fixed_id(),
            tool,
    ...
    // The host's quoter — used in tests so assertions work on both Unix and Windows runners.
    ...
    fn claude_compose_basic() {
        ...
        let r = compose_command(&inputs(Tool::Claude, &wt, "my-feature")).expect("compose");
    ...
    fn copilot_compose_basic() {
        ...
        let r = compose_command(&inputs(Tool::Copilot, &wt, "wt")).expect("compose");
        ...
        // Worktree path must not leak into the composed command — it is supplied to `portable-pty` as `cwd`.
        assert!(!r.composed_command.contains(&*wt.to_string_lossy()));
        assert!(!r.composed_command.contains("cd "));
    }
    ...
    fn copilot_compose_does_not_leak_worktree_path_when_path_contains_spaces() {
        ...
        let r = compose_command(&inputs(Tool::Copilot, &wt, "feature")).expect("compose");
    ...
    fn copilot_compose_uses_cli_launch_command_override() {
        let wt = PathBuf::from(if cfg!(windows) { "C:\\wt" } else { "/wt" });
        let mut i = inputs(Tool::Copilot, &wt, "wt");
    ...
    fn with_resume_appends_verbatim_id_to_bare_claude() {
        ...
        // Slug-safe id: appended verbatim, no shell quoting (avoids `.cmd`-shim quote leakage on Windows).
        assert_eq!(out, "claude --resume abc-123");
    }

    #[test]
    fn with_resume_appends_after_existing_flags() {
        let base = "claude --model sonnet";
    ...
    fn with_resume_quotes_ids_with_shell_metachars() {
        // Defensive: a future name-based resume id must not corrupt the command. The host quoter should fully escape it.
    ...
    fn posix_quote_round_trip_properties(#[case] input: &str) {
        ...
        // The "interior" — between the leading and trailing single quotes — must contain no unescaped single quote. Single quotes inside are encoded
        // as `'\''`, which means the only ' chars we should see inside are part of `'\''` patterns. Concretely: every run of consecutive `'`
        // characters in the encoded output must have even length-or-be-part-of-the-escape: the simplest structural check is that splitting on `'\''`
        // and re-joining recovers the original by replacing back.
    ...
    fn cmd_quote_structural_properties(#[case] input: &str) {
        ...
        // Reserved cmd metacharacters that survive double-quoting must each be preceded by a caret in the encoded form. We verify this by
        // reconstructing the input from `inner`: walking the encoded bytes, every `^X` where `X ∈ {%, !, ^}` collapses to `X`, and every other char
        // is itself. The result must equal the original input modulo the CRT-layer `\"`/`\\` encoding (which we decode separately below).
        ...
        // Now reverse the CRT layer: every `\"` → `"`, every run of `\\` before a `"` or end-of-string halves; bare `\` outside of those contexts is
        // kept. Equivalent operational decode: replace `\"` with `"`, then replace `\\` with `\`.
        let crt_decoded = decoded_cmd.replace("\\\"", "\"");
        // Halve any trailing/embedded doubled backslashes that were doubled because they preceded a `"` or end-of-string. We doubled *all*
        // backslashes that immediately preceded a `"` and any trailing run; for inputs in this test set the simple `\\` → `\` replacement on the
        // remaining string round-trips correctly.
    ...
    fn cmd_quote_preserves_trailing_backslash_before_closing_quote() {
        // `foo\` inside `"…"` must become `foo\\` so the loader doesn't see the `\` as escaping our closing quote.
    ...
    fn dedupe_label_fills_lowest_gap() {
        // Documented rule: pick the lowest free integer >= 2 — so a gap at "foo 2" gets filled even though "foo 3" exists.
    ...
    fn dedupe_label_stress_many_existing() {
        ...
        refs.push("foo");
        // "foo 1" exists in `owned`; the lowest free n >= 2 is 100 since 2..=99 are all taken.
    ...
    fn env_for_tool_copilot_returns_otel_keys() {
        ...
        // Standard OTel SDK env var; literal "1000" (ms) tightens batch flush from the 5s default to ~1Hz.
