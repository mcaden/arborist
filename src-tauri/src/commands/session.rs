...
use crate::types::{
    AppConfig, AppError, Error, InstructionSet, PartialAppConfig, RepoCommandTrustArgs, Session, SessionCloseResult, SessionCreateArgs, SessionId,
    SessionInputArgs, SessionResizeArgs, SessionRestartArgs, SessionStatus, SessionView, ShellCommandIntent, ShellCommandKind, ShellCommandPreview,
...
fn session_create_trust_inputs(
    ...
    tool: Tool,
    worktree_path: &Path,
    instruction_set_id: Option<&crate::types::InstructionSetId>,
    ...
    let id_opt = instruction_set_id.filter(|id| !id.as_str().is_empty());
    let set_opt = match id_opt {
        Some(id) => Some(lookup_instruction_set(&cfg, id, tool)?),
        None => None,
    };
    ...
    let composed = compose::compose_command(&ComposeInputs {
        ...
        cli_launch_command: Some(cfg.ai_launch_command_for_tool(tool)),
    })
...
pub fn shell_command_preview_impl(ctx: &AppContext, args: ShellCommandPreviewArgs) -> Result<ShellCommandPreview, AppError> {
    ...
    match args.intent {
        ShellCommandIntent::SessionCreate {
            tool,
            worktree_path,
            instruction_set_id,
        } => {
            let preview_id = SessionId(uuid::Uuid::nil());
            let (target, cfg, candidates) = session_create_trust_inputs(ctx, tool, &worktree_path, instruction_set_id.as_ref(), preview_id)?;
