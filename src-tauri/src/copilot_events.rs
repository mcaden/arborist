...
//!
//! Copilot writes a structured, append-only event stream to `~/.copilot/session-state/<sessionId>/events.jsonl`. The tailer turns this stream into
//! [`crate::activity::ActivityEvent`]s the frontend's sidebar state machine consumes via `session://activity`. We surface three new sub-states the
//! byte-rate scanner can't tell apart:
//!
...
//!
//! The tailer is **Copilot-only** by design: Claude's transcript JSONL doesn't carry tool/permission events on the same schema. Until Claude exposes
//! hooks (ROADMAP §4.5 / Issue #4), Claude tabs continue using the PTY-byte [`crate::activity::ActivityScanner`].
//!
//! ## Why path-by-session-id (not directory watch)?
//!
//! Pre-allocation decides the conversation uuid at `session_create`, splices it via `--session-id <uuid>` at every spawn (copilot-cli >= 1.0.51
//! switched Arborist from `--resume <uuid>` — see issue #222), and persists it
...
//!
//! Events are append-only and time-ordered, but the tailer **must not** crash on:
...
//!
//! When the watcher starts after a restart (events.jsonl already exists with prior content), we replay the file from byte 0, applying the same state
//! transitions but **suppressing** transient `ToolStart→ToolEnd` and `AwaitingPermission→PermissionResolved` pairs that have already resolved. After
//! catch-up, only the *currently open* state is emitted (a single `AwaitingPermission`, a final `ToolStart`, or `TurnStart` if still in a turn). This
//! keeps the UI quiet during restore.
//!
//! ## Known limitation: `/clear` mid-session
//!
//! Copilot's `/clear` rotates the conversation to a *new* uuid and starts writing to a fresh `events.jsonl` under the new path. The sibling
//! [`crate::session_metrics`] OTel watcher detects the new `gen_ai.conversation.id` and persists it to `Session.ai_session_id`, but **this watcher
//! does not hot-swap its events path** — it stays bound to the original uuid passed to
//! [`spawn_watcher`]. After `/clear`, sidebar sub-states
//! (awaiting-permission / running-tool / thinking) for that session reflect the last known state of the *old* conversation until the user issues a
//! `session_restart` (which tears down and re-spawns the watcher with the rotated id). Token / model / duration metrics continue to update because
//! the OTel watcher is conversation-id agnostic. Tracked as a follow-up; out of scope for the initial Phase 2.5 surface.
...
/// Poll interval — kept in line with [`crate::session_metrics`] so the two watchers tick on the same cadence and don't fight for the disk.
const POLL_INTERVAL: Duration = Duration::from_millis(500);

/// Path to the per-session events.jsonl given a Copilot conversation id. Public so the production caller in `commands::session` can compute it at
/// watcher-start time without re-implementing the layout.
#[must_use]
...
pub struct EventsState {
    /// Are we inside an `assistant.turn_start` ... `assistant.turn_end` bracket? Tracked as a turn id (Some(_) = in turn) rather than a boolean so we
    /// can match start/end pairs and skip stray ends.
...
/// Minimal envelope shared across every event type in events.jsonl. Every concrete event has at least `type` and (optionally) `data`. Unknown type
/// values are silently skipped — the tailer is forward-compatible with new Copilot CLI versions.
...
///
/// `suppress_resolved` controls catch-up behavior: when `true`, we still update internal state for pairs that resolve within the replay (so we don't
/// end up with bogus open counts), but we don't emit transient events for them. Set `true` during the initial mid-file catch-up read and `false` once
/// we're tailing live.
pub fn ingest_line<F: FnMut(ActivityEvent)>(state: &mut EventsState, line: &[u8], suppress_resolved: bool, mut emit: F) {
    ...
    match env.kind {
        ...
        "assistant.turn_end" => {
            ...
            // The metrics watcher is the canonical source of TurnEnd-with-duration (Copilot OTel `invoke_agent` span). We emit a TurnEnd here without
            // a duration so the frontend's reducer flips out of `thinking` promptly even when OTel is delayed or missing.
        ...
        "tool.execution_complete" => {
            ...
            // If we never saw the matching start (e.g. catch-up began mid-file after the start line), drop the end on the floor. The frontend's
            // reducer is also defensive but emitting a bogus ToolEnd would decrement a counter that was never incremented.
        ...
        "abort" => {
            // User pressed ESC (or similar). Treat as turn-end + drop any open tools — they will never complete via tool.execution_complete because
            // the agent was killed mid-call. Permissions stay open; the user's resolution (or lack thereof) is what produced the abort.
        ...
        _ => {
            // Unknown event kinds (session.start, session.shutdown, session.model_change, hook.*, system.message, ...) are intentionally ignored at
            // this layer. The metrics watcher owns model-change handling; lifecycle is owned by the PTY pool. New unknown kinds in future Copilot
            // versions are forward-compatible by default.
        }
    }
}
...
/// Extract the user-visible kind label for a permission request. The Copilot schema isn't public; we accept several shapes seen in the wild and fall
/// back to `"permission"` for anything we don't recognise.
...
/// Extract a one-line summary for a permission request. Best-effort — returns `None` if no shape we recognise is present.
fn extract_permission_summary(data: &serde_json::Value) -> Option<String> {
    // Prefer the nested `permissionRequest` object when present; fall back to the envelope itself for shapes that inline the fields. Always-Some, so
    // use `unwrap_or` rather than `or(Some(_))?`.
...
// events.jsonl. Mirrors `session_metrics::tail_lines` shape so behavior is consistent across watchers.
// ---------------------------------------------------------------------------
...
/// the cursor past a `\n`, so cursor sticks just after the previous complete line and `cursor >= target` never holds. Result: the watcher silently
/// stops working for that session — no live emissions, no `emit_current_state` synthesis. The only escape would be a `session_restart`.
///
/// The fix: snap the catch-up target to the byte just past the last `\n` at-or-before `len`. The trailing partial bytes (if any) get re-read in the
/// live phase once they are completed by the next write — and emitted with `suppress=false`, which is the correct classification (the user *is*
/// watching that event happen).
///
...
pub(crate) fn align_snapshot_to_line_boundary(path: &std::path::Path, len: u64) -> u64 {
    ...
    // Scan backward in 64 KB chunks. Lines should comfortably fit; if we can't find a `\n` in the entire file, the whole content is one partial line
    // and returning 0 is correct.
...
/// Run the events.jsonl tailer for one Copilot session. Blocking — call from a dedicated OS thread (see [`spawn_watcher`]).
///
...
pub fn run_watcher(session_id: SessionId, events_path: PathBuf, emit: CopilotActivityCb, running: Arc<AtomicBool>) {
    ...
    // EOF snapshot taken on the first poll iteration (before any reads). Catch-up "done" means cursor has drained past this snapshot — *not* "we read
    // at least one chunk". This matters because `tail_lines_pub` caps each read at `MAX_READ_CHUNK` (10 MB), so on a large pre-existing events.jsonl
    // a single iteration won't drain the file. Without this guard, history past the first chunk would be emitted with `suppress=false` and surface as
    // spurious live tool/permission flickers in the sidebar after restore.
    let mut catch_up_target: Option<u64> = None;
    // Track which tools / permissions we've *already announced* on the wire, so a second emission after a rotate-and-replay doesn't double-count. The
    // frontend reducer is also defensive (idempotent on matching ids), but local dedup avoids unnecessary cross-process chatter.
    ...
    while running.load(Ordering::SeqCst) {
        if let Ok(meta) = std::fs::metadata(&events_path) {
            let len = meta.len();
            // Truncated/rotated under us: reset and re-snapshot. Same handling as session_metrics's OTel watcher.
            if len < cursor {
                cursor = 0;
            ...
            // Snapshot the catch-up target on the first iteration that sees the file. Subsequent iterations keep the original target so events
            // appended *after* catch-up started are treated as live (emitted), not catch-up (suppressed).
            //
            // The raw EOF can land mid-line (interrupted prior write or active flush race); aligning to the last complete-line boundary keeps `cursor
            // >= target` reachable and prevents the watcher from wedging in catch-up forever. See
            ...
            // During catch-up, cap the read end at the snapshot so any bytes appended after we started are deferred to the live phase. After catch-up
            // flips, read up to the current EOF.
            ...
            if read_end > cursor {
                ...
                cursor = crate::session_metrics::tail_lines_pub(&events_path, cursor, read_end, |line| {
                    ingest_line(&mut state, line, suppress, |ev| {
                        // Defensive de-dup on tool/permission ids for live-tail emissions only — the catch-up path doesn't emit at all
                        // (suppress=true).
            ...
            // Catch-up flips done only once cursor has drained past the pre-read EOF snapshot. For empty / no-new-bytes files the condition is
            // trivially true (target == cursor == 0 or cursor already past target).
            if !catch_up_done {
...
/// Spawn a dedicated OS thread running [`run_watcher`]. Returns the handle so callers can `join` for quiescence (mirrors
...
mod tests {
    ...
    fn catch_up_suppresses_resolved_pairs_and_emits_only_open_state() {
        // Catch-up: replay a complete history with `suppress=true`. The tailer should track state correctly but emit nothing during replay;
        // emit_current_state then surfaces only what's still pending.
        let mut s = EventsState::new();
    ...
    // ----------------------------------------------------------------- End-to-end watcher harness — drives a real file from a background thread,
    // asserts the emitted events. -----------------------------------------------------------------
    ...
    fn run_watcher_emits_live_events_after_catch_up() {
        ...
        // Wait until catch-up has finished (no events from quiescent history) — tail-poll cycle is ~POLL_INTERVAL.
    ...
    fn run_watcher_catch_up_synthesizes_pending_permission() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("events.jsonl");
        // Pre-seed with an *unresolved* permission request — catch-up should synthesize a single AwaitingPermission so the sidebar reflects current
        // state on restore.
        std::fs::write(
            &path,
            b"{\"type\":\"assistant.turn_start\",\"data\":{\"turnId\":\"t0\"}}\n\
    ...
    fn run_watcher_catch_up_over_max_chunk_does_not_emit_historical_events() {
        // Regression for the multi-chunk catch-up bug. `tail_lines_pub` (and the underlying `read_range`) caps each read at 10 MB (`MAX_READ_CHUNK`).
        // The pre-fix code flipped `catch_up_done` after the *first* read, so any history past the 10 MB mark was emitted with `suppress=false` —
        // i.e., as if it were live — surfacing as spurious tool/permission flickers in the sidebar immediately after a Copilot session was restored.
        //
        // The fix snapshots EOF on the first iteration and only flips `catch_up_done` once cursor has drained past that snapshot. We seed
        // events.jsonl with ~10 MB of forward-compatible padding lines (unknown event type — silently ignored by `ingest_line`), then a
        // fully-resolved tool pair *past* the 10 MB boundary. With the bug, the resolved pair surfaces as a live ToolStart/ToolEnd. With the fix,
        // neither event fires.
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("events.jsonl");
        ...
        let bytes_per_line: u64 = (padding_line.len() + 1) as u64; // newline
                                                                   // Push clearly past MAX_READ_CHUNK (10 MB) so a single poll can't drain it in one
                                                                   // read. ~11 MB of padding → second poll is required to reach EOF. Test takes ~2
                                                                   // polls × 500 ms.
        ...
        use std::io::{BufWriter, Write};
        {
            ...
            // Resolved tool pair past the 10 MB boundary. With the bug these lines are read in iteration 2 with suppress=false and surface as live
            // emissions.
        ...
        // Wait for several poll cycles so multi-chunk catch-up fully drains. POLL_INTERVAL is 500 ms; 5 cycles == 2.5 s, enough even on a slow CI
        // runner to cover at least three reads (initial → drain past 10 MB → flip `catch_up_done`).
    ...
    // ---------------------------------------------------------------- The catch-up snapshot used to be raw `metadata().len()`, which could land
    // inside a partial trailing line if the prior Copilot session had been interrupted mid-write or if we polled while a line was being flushed.
    // `tail_lines_pub` only advances cursor past a `\n`, so cursor would stick just after the previous complete line and `cursor >= target` would
    // never hold — wedging catch-up forever, with no live emissions and no `emit_current_state`.
    //
    // The fix aligns the snapshot to the last `\n` at-or-before EOF (see `align_snapshot_to_line_boundary`). These tests pin the three shapes the bug
    // took.
    ...
    fn align_snapshot_truncates_to_last_newline_when_partial_trails() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("events.jsonl");
        // Two complete lines (4 bytes "a\nb\n") + a partial trailing "xxx" with no newline. Snapshot must land on byte 4 so the partial bytes are
        // deferred to the live phase.
        std::fs::write(&path, b"a\nb\nxxx").unwrap();
    ...
    fn run_watcher_does_not_wedge_when_snapshot_lands_in_partial_trailing_line() {
        // Reproduces the pre-fix hang: events.jsonl ends with a partial (no-`\n`) trailing line at watcher-start. Pre-fix, the catch-up target
        // snapped to mid-line, cursor stuck past the previous complete line, and a later append (which completed the partial line + added a fresh
        // resolved tool pair) was never emitted because `read_end` stayed clamped to the mid-line target. With alignment the target snaps to the
        // boundary just after the last complete `\n`, the partial trailing bytes get re-read in the live phase once they are completed by the next
        // write, and any subsequent fresh events flow through normally.
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("events.jsonl");
        // Pre-seed: one complete, fully-resolved tool pair (catch-up suppresses these) + a partial trailing event (no `\n`).
        std::fs::write(
            &path,
        ...
        // Now append: complete the partial line, then add a fresh resolved permission request that the watcher must surface as a live event.
        let mut f = std::fs::OpenOptions::new().append(true).open(&path).unwrap();
        f.write_all(b"ell\"}}\n").unwrap();
        f.write_all(
    ...
    fn run_watcher_completes_catch_up_with_only_partial_trailing_content() {
        // File is one partial line, never completed, no further writes. Pre-fix: catch-up wedged forever. Post-fix: snapshot aligns to 0,
        // catch_up_done flips immediately, emit_current_state runs (no-op for empty state), and the watcher reaches live mode ready to receive the
        // next append.
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("events.jsonl");
        std::fs::write(
        ...
        // Append a fresh, complete line — must be emitted live (proves catch-up didn't wedge).
        let mut f = std::fs::OpenOptions::new().append(true).open(&path).unwrap();
        f.write_all(b"ell\"}}\n").unwrap();
        drop(f);
