...
//!
//! When a (branch, workspace) tuple is opened for the first time, its
//! storage dir is empty. To make the user experience smooth across an
//! upgrade — and to avoid forcing every branch dev build to start with
...
//!
//! The marker for "this dir has already been seeded" is the existence
//! of `<workspace_dir>/workspace-meta.json`, which is always written
//! by [`initialise_workspace_dir`] regardless of which sources matched.
//! Using a dedicated marker (rather than overloading `config.json`)
//! avoids a corner case where a branch build with no canonical seed
//! source would re-attempt the seed every launch.
//!
//! ## Concurrency
//!
//! Two Arborist processes can race to seed the same dir on first
...
//! workspace lock acquired by `WorkspaceScope` (a separate file).
...
/// What [`initialise_workspace_dir`] did (or didn't) for a given
/// invocation. Carried back to the caller so boot-time logging can
/// record the choice without having to re-stat the filesystem.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SeedOutcome {
    /// `workspace-meta.json` already existed — the dir was previously
    /// initialised, and no source files were copied.
    AlreadySeeded,
    /// `config.json` was copied from the canonical build's same-
    /// workspace settings (only possible for branch builds).
    SeededConfigFromCanonical,
    /// `config.json` was copied from the legacy top-level path
    /// (`<app_data_dir>/config.json`).
    SeededConfigFromLegacy,
    /// `sessions.json` was copied from the legacy top-level path
    /// (`<app_data_dir>/sessions.json`). May appear *in addition to*
    /// a `SeededConfig*` outcome — see [`SeedReport`].
    SeededSessionsFromLegacy,
    /// No seed source applied; the dir is fresh and only the
    /// `workspace-meta.json` sidecar was written.
    Fresh,
}

/// Aggregate outcome for one [`initialise_workspace_dir`] call.
/// Multiple outcomes can co-occur (e.g. config from canonical, sessions
/// from legacy), so the report is a vector. Empty vector means no
/// seeding happened (`AlreadySeeded` is signalled via
/// [`Self::already_seeded`]).
#[derive(Debug, Default)]
pub struct SeedReport {
    /// `true` iff the marker was already present and we returned
    /// without copying anything. Mutually exclusive with all other
    /// outcomes.
    pub already_seeded: bool,
    /// What was copied (in source-priority order). Empty for both
    /// `AlreadySeeded` and `Fresh`-only invocations.
...
#[derive(Debug)]
pub enum SeedError {
    /// Failed to acquire the seed lock (e.g. permissions on
    /// `.config-seed.lock`'s parent dir). Distinct from a lock
    /// contention timeout — `acquire_blocking` waits indefinitely.
    Lock(LockError),
...
/// file is purely informational — the marker test is `path.exists()`,
/// not the parsed contents.
...
/// Subset of `AppConfig` we need to read from a candidate seed source
/// to verify its `workspaceRoot` matches the dir being seeded.
/// Defined inline (not reusing `AppConfig`) so an unrelated schema
/// change in the source file can't break seed compatibility.
...
/// Initialise (idempotently) the storage dir for `layout`, performing
/// the seed-on-first-launch logic described at the module level.
/// Safe to call concurrently from multiple processes; only one will
/// win the seed and the others return [`SeedReport::already_seeded`].
///
/// Always creates `layout.workspace_dir()` if missing, and always
/// writes `workspace-meta.json` on the winning path so subsequent
...
pub fn initialise_workspace_dir(layout: &StoreLayout) -> Result<SeedReport, SeedError> {
    ...
    // ---- sessions.json ----
    // Only canonical builds seed sessions. Branch dev builds start
    // with an empty session list per SPEC §C-04 so feature-branch
    // experiments don't entangle with the user's "real" session set.
    ...
    // ---- marker ----
    // Use `OpenOptions::create_new` to write the marker. Combined with
    // the seed lock above, this gives belt-and-suspenders atomicity:
    // even if a buggy caller skipped the lock, two processes cannot
    // both successfully write the marker. The loser sees
    // `ErrorKind::AlreadyExists` and reports `AlreadySeeded` instead
    // of double-seeding.
...
/// Branch builds: `canonical_workspace_settings_path` may return
/// `Some(...)` even if that file doesn't exist. Caller must check
/// `exists()` before copying. (Module-internal helper, kept private.)
///
...
fn should_seed_from_legacy_config(legacy_path: &Path, layout: &StoreLayout) -> bool {
    ...
    match read_legacy_workspace_root(legacy_path) {
        ...
        None => {
            ...
            // warning rather than propagating the error. The store
            // will simply start fresh; the user can re-import later.
            warn!(
...
/// For sessions seeding, we need `workspaceRoot` to match — the same
/// rule applies but without the canonical-build "adopt" relaxation
/// (sessions are tied to a specific workspace's worktrees). Reads
/// the *config* to check, since `sessions.json` itself doesn't carry
/// `workspaceRoot`.
...
/// `Some(Some(path))` → field present and set.
/// `Some(None)`       → field present and explicitly null/absent (default).
/// `None`             → couldn't read or parse the file.
...
/// Compare two paths for "are these the same workspace?" semantics.
/// Both should be canonicalised by the caller; we still apply
/// `dunce::canonicalize` defensively for the legacy path which was
/// written by an older binary that may not have canonicalised.
...
/// Copy `src` → `dest` via a same-directory `NamedTempFile::persist`,
/// matching `config_store::write_atomic`'s durability story (rename
/// is atomic on the same filesystem). `dir` is the parent of `dest`
...
fn write_marker_create_new(layout: &StoreLayout, dest: &Path) -> io::Result<()> {
    ...
    // `create_new(true)` returns `ErrorKind::AlreadyExists` if the
    // marker is present, which the caller treats as the loser-path
...
mod tests {
    ...
    /// files are created. Re-running is idempotent.
    #[test]
    ...
    /// workspace: don't seed (would mix unrelated state).
    #[test]
    ...
    /// Canonical build, legacy config exists with no `workspaceRoot`:
    /// adopt-on-first-pick. Sessions still need the explicit match,
    /// so they are NOT seeded here.
    #[test]
    ...
    /// Branch build with no canonical settings AND legacy with unset
    /// `workspaceRoot`: do NOT adopt — adopt only applies to
    /// canonical builds. Result is Fresh.
    #[test]
    ...
    /// Two threads racing to seed the same dir: exactly one performs
    /// the copy, the other returns `already_seeded`. Proves the
    /// blocking lock + lock-then-recheck pattern.
    #[test]
    ...
    /// Marker presence short-circuits everything, even if a juicier
    /// seed source appears later. Proves the marker is the source of
    /// truth, not the contents of `config.json`.
    #[test]
    fn marker_short_circuits_subsequent_seeds() {
        ...
        // Now create a legacy seed source. A subsequent init MUST NOT
        // re-seed because the marker is already there.
        touch_json(
            &root.legacy_config_path(),
            &serde_json::json!({
                "configVersion": 4,
    ...
    /// Malformed legacy config must not propagate as an error — the
    /// dir is initialised fresh and a warning is logged.
    #[test]
    ...
    /// Belt-and-suspenders defence: even if the seed lock weren't
    /// effective, a pre-existing marker must not be overwritten by
    /// `write_marker_create_new`. We pre-write a marker without
    /// taking the lock and then call `initialise_workspace_dir`; it
    /// must report `AlreadySeeded` because the `create_new(true)` on
    /// the marker write fails with `AlreadyExists`. Proves the
    /// second-line-of-defence the reviewer asked for.
    #[test]
