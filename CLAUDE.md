...
- **Copilot**: do **not** pass `--instructions` — it disables auto-discovery of `.github/copilot-instructions.md`. **Minimum CLI version: `copilot >= 1.0.51`** (released 2026-05-20). Arborist uses `--session-id <uuid>` for first launch, restart, and restore-on-launch (create-or-resume). Older Copilot CLIs used `--resume <uuid>` for the same create-if-absent semantic but copilot-cli 1.0.51 deliberately split the two flags — `--resume` is now strict-resume-only and errors on unknown UUIDs, so Arborist would fail to start any new Copilot session against an older CLI build. Upstream split: https://github.com/github/copilot-cli/issues/3377.
- **Copilot session id**: pre-allocated at create time (mirrors Claude). The persisted `composed_command` stays bare; `compose::with_resume` splices `--session-id <uuid>` at every spawn (create, restart, restore-on-launch) via `CopilotPlugin::resume_args`. Restart uses `RestartAiSessionPolicy::Preserve` so the SAME conversation continues across an in-app restart (matching Claude's restart behaviour) instead of starting fresh — users who want a clean conversation use the CLI's own `/clear`.
