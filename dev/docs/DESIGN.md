...
interface AppConfig {
  ...
  worktreesDir: string; // Parent folder for new worktrees (default '.worktrees'); relative → resolved against workspaceRoot, absolute → used verbatim (Issue #53)
...
- `5` — added `worktreesDir` (Issue #53). Pre-v5 configs hydrate the field
  with the default `.worktrees` via serde. Empty / whitespace-only values
  normalise to the default at both save (`merge_partial`) and load
...
| `worktree_create` | `{ name: string }` | `{ path: string }` | Create a new linked worktree at `<worktreesParent>/<name>` on a fresh branch named `<name>` (Roadmap §2.2 / Issue #53), where `<worktreesParent>` is `AppConfig.worktreesDir` resolved against the workspace root (relative — symlink/containment guard applies) or used verbatim (absolute — only the no-clobber and parent-not-symlink guards apply). Requires `workspaceRoot` to be set in `AppConfig`; errors with `NotFound` otherwise. The `name` is re-validated server-side via the same rules as `validateWorktreeName` (no spaces; no `..`, `~`, `^`, `:`, `?`, `*`, `[`, `\\`; cannot start/end with `.` or `/`; cannot end with `.lock`; cannot be `@`; 1–255 chars); `InvalidPath` is returned for any rule violation. Runs `git -C <workspaceRoot> worktree add <worktreesDir>/<name> -b <name>` (relative case) or `git ... worktree add <abs-parent>/<name> -b <name>` (absolute case) via the injected `GitRunner`; bubbles up the captured stderr in the `Internal` error message on git failure. Returns the canonical absolute path to the new worktree directory. |
| `worktrees_dir_check` | `{ value: string }` | `{ resolvedPath: string \| null, insideRepo: boolean, gitIgnored: boolean }` | Live preview helper for the Settings dialog (Issue #53). Lexically resolves `value` against the active `workspaceRoot` (no filesystem touch, so paths that don't exist yet still preview correctly), reports whether the result is inside the workspace, and — only when inside — probes Git via `git -C <workspaceRoot> check-ignore -q --no-index --` for both `<dir>` and `<dir>/.arborist-ignore-probe` (covers `.gitignore` rules that match files vs directories). Returns `{ resolvedPath: null, insideRepo: false, gitIgnored: false }` when no workspace is bound. Performs no mutation; safe to call on every keystroke (the dialog still debounces ~250ms). |
