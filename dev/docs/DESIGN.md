...
8. Reset the `restored` atomic so the inline restore (step 10) and any
   later defensive `frontend_ready` re-issue from the new mount can
   re-arm consistently. The atomic is **re-latched to `true`** in
   step 10 after the inline restore completes, so a stray
   `frontend_ready` cannot trigger double-spawn.
...
10. **Best-effort post-swap hint write.** Persist
    ...
    this hint lands.
11. **Inline restore.** Run `restore_all_sessions` against the
    ...
    restore completes, latch `restored = true` so a frontend-initiated
    `frontend_ready` cannot re-fire restore against the same
    workspace. The write guard is held for the entire restore so no
    new lifecycle handler can interleave; restore registers each
    session as a deferred-spawn entry whose `pool.spawn` fires from
    the first `session_resize` after the switch returns.
12. Build the post-switch snapshot — load `AppConfig` + the persisted
...
by step 7; the new workspace's sessions are restored inline by step 11
