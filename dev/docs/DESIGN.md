...
`dev/docs/CONFIGURATION.md`. The same per-(branch, workspace) `config.json`
holds the v4 `customProcesses` and `lastOpenSubSessions` arrays; sub-session
restore is therefore scoped to the active workspace exactly like top-level
sessions.
...
| `worktree_create` | `{ name: string }` | `WorktreeCreateResult = { path: string, prep: WorktreePrepInfo \| null }` | Create a new linked worktree at `<workspaceRoot>/.arborist/.worktrees/<name>` on a fresh branch named `<name>` (Roadmap §2.2, issue #71). Requires `workspaceRoot` to be set in `AppConfig`; errors with `NotFound` otherwise. The `name` is re-validated server-side via the same rules as `validateWorktreeName` (no spaces; no `..`, `~`, `^`, `:`, `?`, `*`, `[`, `\\`; cannot start/end with `.` or `/`; cannot end with `.lock`; cannot be `@`; 1–255 chars); `InvalidPath` is returned for any rule violation. As a side-effect, ensures `<workspaceRoot>/.arborist/` exists and contains a `.gitignore` listing `.worktrees/`. Runs `git -C <workspaceRoot> worktree add .arborist/.worktrees/<name> -b <name>` via the injected `GitRunner`; bubbles up the captured stderr in the `Internal` error message on git failure. Returns the canonical absolute path to the new worktree directory. **Worktree prep (issue #63):** when `AppConfig.worktreePrepCommands` (with `<workspaceRoot>/.arborist/settings.json` overlaid per issue #71) has at least one non-blank entry, the wrapper additionally spawns a one-shot prep job in the new worktree's cwd. The `prep` field is `null` when no commands are configured; otherwise it carries `{ prepId, worktreePath, logPath }`. Lifecycle (`started` / `exited`) flows through the `worktree://prep` event. Spawn-failure of the prep itself is **not** a `worktree_create` failure — the create still resolves with `prep: Some(...)` and the failure surfaces as an `exited` event with `exitCode: null` plus a populated `errorMessage`. |
...
| `worktree_git_status` | `{ path: string }` | `WorktreeGitStatus = { head: string \| null, branch: string \| null, upstream: string \| null, ahead: number, behind: number, staged: number, unstaged: number, untracked: number, conflicted: number, files: GitStatusFile[], filesTruncated: boolean, error: string \| null }` | Snapshot of `git status --porcelain=v2 --branch -z --untracked-files=all` for the parent dashboard's Git Status panel (issue #55). **Always resolves Ok** — a missing/invalid path or git-runner failure is folded into a successful result with `error: Some(...)` and every count zeroed, so the frontend can distinguish "clean tree" (`error == null`, all counts 0) from "unreadable" (`error` populated) without a rejected `invoke`. Path validation runs `compose::validate_worktree` (exists + canonicalize + is_dir); the canonical path is then passed as `cwd` to the `GitRunner` — never interpolated into a command. `files` is best-effort capped at `MAX_GIT_STATUS_FILES` and `filesTruncated` flags when the cap kicked in; counts are always whole-repo regardless of truncation. Untracked-file enumeration uses `--untracked-files=all` so each file inside an entirely-untracked directory is counted individually instead of git's default `?? dir/` shorthand. Polled every 15 s by the dashboard with a frontend in-flight guard. |
...
│   │   ├── store_layout.rs       # Per-(branch, workspace) StoreRoot / StoreLayout primitives
│   │   ├── workspace_lock.rs     # fs2 exclusive lock guarding the active workspace store dir
│   │   ├── workspace_scope.rs    # WorkspaceScope held under RwLock; live store + lock binding
│   │   ├── seed.rs               # First-launch seed-on-pick (legacy / canonical / branch builds)
│   │   ├── boot.rs               # Workspace resolution chain + log-guard-safe failure exits
│   │   ├── commands/             # #[tauri::command] handlers
│   │   │   ├── mod.rs            #   wiring + production sinks + AppContext + workspace_switch
│   │   │   ├── session.rs        #   session_* + restore + workspace switch gating
...
│   │   ├── allow-subsession-icon.toml  # Tauri capability allow-list for subsession_icon
│   │   └── allow-workspace-switch.toml # Tauri capability allow-list for workspace_switch
│   ├── capabilities/
