...
The sidebar is a two-level hierarchy: each top-level row is a **worktree tab** (issue #44); its child rows are the AI-agent sessions and any custom-process sub-sessions whose `worktreePath` matches the tab. Children render flat under their parent (sub-sessions are still scoped to their owning agent session by `parentSessionId`, but visually they sit alongside their siblings under the worktree row).

| ID     | Requirement |
|--------|-------------|
| S-01   | The sidebar MUST be displayed as a vertical tab bar on the left edge of the window. |
| S-02   | Each child tab MUST display the tool icon (Claude or Copilot logo) and the session label. Each worktree-tab row MUST display the worktree name, optional branch, and a status badge that rolls up the worst-priority child status. |
...
| S-04   | The sidebar MUST include a "+" button (or equivalent) to create a new session. The new-session flow auto-creates the parent worktree tab if one does not already exist for that path, and auto-links the resulting session as the parent's `activeChildId`. |
...
| S-09   | Right-clicking a worktree-tab row MUST present a context menu with **Close** and **Launch ▸** (Claude / Copilot) entries. Selecting an agent invokes session creation against the worktree path, materialising a new child under that worktree tab. |
